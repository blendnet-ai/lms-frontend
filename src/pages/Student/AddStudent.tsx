...
const AddStudent = () => {
  ...
  const handleUpload = async () => {
    ...
    setLoading(true);
    try {
      const resp = await BULKAPI.uploadStudents(file);
      setData(resp);
    ...
    <div className="flex flex-col min-h-screen w-full bg-[#EFF6FF] p-8 pt-6">
      ...
      <div className="flex flex-col bg-white gap-5 mb-5 p-5 pl-6 mt-4 w-full">
        <p className="text-base text-gray-500 w-1/2">
          Upload a XLSX file containing the list of students. The file should
          ...
          <span className="font-semibold">Training Location District Name</span>
          ,<span className="font-semibold">Training Location City Name</span>,{" "}
          <span className="font-semibold">Course Code</span>,{" "}
          <span className="font-semibold">Batch ID</span>,{" "}
          <span className="font-semibold">Onboarding Source</span>,{" "}
        ...
        {data && (
          <span className="text-base font-semibold text-[#2059EE]">
            {`Upload status: ${data.data.success_count} students added successfully, ${data.data.failed_count} failed.`}
          </span>
        )}
